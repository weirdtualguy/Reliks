...
(async () => {
  const js = fs.readFileSync('examples/bloom3.js', 'utf8');
  if (Buffer.byteLength(js) > 561) { console.log('❌ JS source > 561 bytes for byte[568] slot — minify or await Series v4'); process.exit(1); }
  const slot = Buffer.alloc(568); slot[0] = 0x4a; slot.writeUInt16BE(Buffer.byteLength(js), 1); slot.write(js, 3, 'utf8');
  const f = compileSeries({ slot: slot.toString('hex'), artist: USER, price: 100000000, cap: 64, role: 0, counter: 0, owner: USER });
  fs.writeFileSync('factory3-program.hex', f.bytecodeHex);
  let d: any = null;
  for (let i = 0; i < 4 && !d; i++) {
  ...
  fs.writeFileSync('factory3.json', JSON.stringify({ covenantId: d.covenant_id, genesis, counter: 0, slotHex: slot.toString('hex') }, null, 1));
