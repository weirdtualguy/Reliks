...
function broadcast(rpcTx) {
  ...
  return new Promise(resolve => {
    const tryUrl = k => {
      ...
      ws.on('message', d => { const s = d.toString(); clearTimeout(t); let txId = null, msg = ''; try { const j = JSON.parse(s); txId = (j.params || j.result || {}).transactionId || null; msg = (j.error && j.error.message) || s.substring(0, 200); } catch (e) { msg = s.substring(0, 200); } ws.close(); resolve({ txId, msg }); });
...
(async () => {
  ...
  const OVER = ALLOW + 1;            // 999001 > remaining 999000
  const LIE = 0;                     // claimed next allowance; accounting says -1
  ...
  const inputs = [
    { txId: L.mintTxId, index: 0, sequence: 0, spk: redeemA(curTok), amount: DUST },
    { txId: L.mintTxId, index: 2, sequence: 0, spk: redeemC(curMin), amount: DUST }
  ];
  ...
  inputs.push(wIn);
  const outputs = [
    { amount: DUST, scriptPublicKey: redeemA(tokState(C, 2, 0, true)), covenant: { authorizingInput: 0, covenantId: A } },
    { amount: DUST, scriptPublicKey: redeemA(tokState(USER, 0, OVER, false)), covenant: { authorizingInput: 0, covenantId: A } },
    { amount: DUST, scriptPublicKey: redeemC({ tokenCovid: A, amount: LIE, initialized: true }), covenant: { authorizingInput: 1, covenantId: C } },
    { amount: wIn.amount - DUST - FEE, scriptPublicKey: wIn.spk }
  ];
  const ssA = B.concat([
    pushMin(B.concat([H(C), H(USER)])), pushMin(B.from([2, 0])), pushMin(B.concat([sm8(0), sm8(OVER)])), pushMin(B.from([1, 0])),
    pushMin(B.from([])), pushMin(B.from([1])), pushMin(TAG_T), pushMin(B.concat([T.prefix, encState(T, curTok), T.suffix]))
  ]);
  const sig1 = secp.schnorr.signSync(sighash(inputs, outputs, 1), PRIV);
  const ssC = B.concat([
    pushMin(H(A)), pushMinInt(LIE), pushMinInt(1),
    pushMin(B.concat([sig1, B.from([0x01])])),
    pushMin(H(C)), pushMin(B.from([2])), pushMinInt(0), pushMinInt(1),
    pushMin(H(USER)), pushMin(B.from([0])), pushMinInt(OVER), pushMinInt(0),
    pushMin(TAG_M), pushMin(B.concat([M.prefix, encState(M, curMin), M.suffix]))
  ]);
  const sig2 = '41' + hex(secp.schnorr.signSync(sighash(inputs, outputs, 2), PRIV)) + '01';
  const rpcTx = {
    version: 1,
    inputs: [
      { previousOutpoint: { transactionId: L.mintTxId, index: 0 }, signatureScript: hex(ssA), sequence: 0, sigOpCount: 0, computeBudget: 60 },
      { previousOutpoint: { transactionId: L.mintTxId, index: 2 }, signatureScript: hex(ssC), sequence: 0, sigOpCount: 0, computeBudget: 60 },
  ...
  if (res.txId) { console.log('❌ NEGATIVE TEST FAILED: over-mint ACCEPTED as ' + res.txId); process.exit(1); }
  if (/verification failed|Rejected/i.test(res.msg)) { console.log('✅ OVER-MINT REJECTED (negative certificate): ' + res.msg); }
  else { console.log('️ unexpected rejection reason: ' + res.msg); process.exit(1); }
