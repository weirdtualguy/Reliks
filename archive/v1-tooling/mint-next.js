...
async function main() {
  ...
  const LEDGER = JSON.parse(fs.readFileSync('factory-ledger.json', 'utf8')); const FACT_TX = LEDGER.txId; const COUNTER = LEDGER.counter;
  let FACT_SPK = '';
  ...
  ws.on('message', d => {
    ...
    if (s.includes('transactionId')) {
      ...
      LEDGER.txId = txId; LEDGER.index = 0; LEDGER.counter = COUNTER + 1; fs.writeFileSync('factory-ledger.json', JSON.stringify(LEDGER, null, 2)); console.log('🌸 EDITION #' + COUNTER + ' MINTED! ledger updated.');
