...
function pushMinimalInt(v) {
  if (v === 0) return B.from([0x00]); if (v >= 1 && v <= 16) return B.from([0x50 + v]);
...
async function fetchRetry(url, opts, n) { n = n || 4;
  for (let i = 0; i < n; i++) { try { return await fetch(url, opts); }
    catch (e) { if (i === n - 1) throw e; console.log('  network drop, retry ' + (i + 1) + '...'); await new Promise(r => setTimeout(r, 2500)); } } }
...
const parts = abi => { const c = abi.contracts[Object.keys(abi.contracts)[0]];
  const bc = B.from(c.compiled.bytecode); const s = c.compiled.state_span;
  return { c, bc, prefix: bc.subarray(0, s.offset), suffix: bc.subarray(s.offset + s.len) }; };
...
(async () => {
  const F = parts(JSON.parse(fs.readFileSync('factory-abi-cap2.json', 'utf8')));
  const E = parts(JSON.parse(fs.readFileSync('edition-abi-v3.json', 'utf8')));
  const LEDGER = JSON.parse(fs.readFileSync('factory-ledger-cap2.json', 'utf8'));
  if (!LEDGER.txId) { console.error('ledger missing genesis txId; wait for indexer then re-run'); process.exit(1); }
  const COUNTER = LEDGER.counter;
  const ART = fs.readFileSync('art-program.hex', 'utf8').trim();
  const artHash = hex(blake2b(H(ART), { dkLen: 32 }));
  const fState = c => ({ art: ART, artist: USER, price: 100000000, royalty_bips: 500, cap: 2, counter: c });
  const curFSpk = p2sh(B.concat([F.prefix, encState(F, fState(COUNTER)), F.suffix]));
  let futxo;
  for (let i = 0; i < 24; i++) {
  ...
  const eState = { ownerIdentifier: USER, identifierType: 0, price: 0, artist: USER,
    royalty_bips: 500, program_hash: artHash, factory_covid: LEDGER.covenantId, serial: COUNTER };
  ...
  const outputs = [
    { amount: fAmt, scriptPublicKey: p2sh(B.concat([F.prefix, encState(F, fState(COUNTER + 1)), F.suffix])), covenant: { authorizingInput: 0, covenantId: LEDGER.covenantId } },
  ...
  // V3 mint sigscript: buyerIdentifier, paymentOutIdx, editionOutIdx, tag, redeem
  const sig0 = B.concat([
    pushExplicit(H(USER)),       // buyerIdentifier (byte[32])
    pushMinimalInt(2),           // paymentOutIdx (int) - artist payment is output 2
    pushMinimalInt(1),           // editionOutIdx (int) - edition is output 1
    pushExplicit(tag),
    pushExplicit(B.concat([F.prefix, encState(F, fState(COUNTER)), F.suffix]))
  ]);
  ...
  if (txId) {
    ...
    fs.writeFileSync('factory-ledger-cap2.json', JSON.stringify(LEDGER, null, 2));
    console.log('🌸 v3 EDITION #' + COUNTER + ' MINTED:', txId);
    console.log('edition covenant:', edCov);
    console.log('🔗 https://kascov.io/testnet-10/tx/' + txId);
  } else console.error('❌ broadcast failed on both endpoints');
