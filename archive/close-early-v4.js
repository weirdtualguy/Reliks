...
const F = JSON.parse(fs.readFileSync('factory-abi-v4.json', 'utf8'));
...
(async () => {
  const L = JSON.parse(fs.readFileSync('factory-ledger-v4.json', 'utf8'));
  const ART = fs.readFileSync('art-program.hex', 'utf8').trim();
  const fState = k => ({ art: ART, artist: USER, price: 100000000, royalty_bips: 500, cap: 64, counter: k });
