...
(async () => {
  const F = parts(JSON.parse(fs.readFileSync('factory-abi-v2.json', 'utf8')));
  const E = parts(JSON.parse(fs.readFileSync('edition-abi-v2.json', 'utf8')));
  const LEDGER = JSON.parse(fs.readFileSync('factory-ledger-v2.json', 'utf8'));
  ...
  const sig0 = B.concat([pushExplicit(H(USER)), pushMinimalInt(2), pushMinimalInt(1), pushExplicit(tag),
  ...
  if (txId) {
    ...
    console.log('🌸 v2 EDITION #' + COUNTER + ' MINTED:', txId);
