...
const compileState = (owner: string, price: number) => compileV2({ collectionId: CID, tokenId: state.tokenId, artPayload: ART, initOwner: owner, initScheme: 0, initPrice: price, initRoyalty: USER });
const out = (v: bigint, spk: string, cov?: any): TxOutput & { covenant?: any } => ({ amount: v, scriptPublicKey: spk, ...(cov ? { covenant: cov } : {}) });

(async () => {
  const cont = compileState(USER, 0);                       // buyer = USER for the demo swap
  ...
  const inputs: UtxoInput[] = [{ txId: state.txid, index: 0, sequence: 0, spk: state.spk, amount: coinValue }, w];
  const outputs = [
    out(coinValue - 1000000n, cont.scriptPublicKey, { authorizingInput: 0, covenantId: state.covenantId }),
    out(PRICE, `20${USER}ac`),
    out(w.amount - PRICE - 1000000n, `20${USER}ac`)];
  ...
  const sb = new SB();
  sb.data(Buffer.from(sig0, 'hex')); sb.data(Buffer.from(USER, 'hex')); sb.data(Buffer.from([0])); sb.int(PRICE); sb.int(1n); sb.data(TAG_SELL); sb.data(Buffer.from(state.programHex, 'hex'));
  const tx = { version: 1, inputs: [
      { previousOutpoint: { transactionId: state.txid, index: 0 }, signatureScript: sb.hex(), sequence: 0, sigOpCount: 0, computeBudget: 15 },
  ...
  console.log('✅ plan.json (atomic sell @ 1 TKAS) ready');
})();
