...
const COV = process.argv[2], PROG = fs.readFileSync(process.argv[3], 'utf8').trim();
const SPK = 'aa20' + Buffer.from(blake2b(Buffer.from(PROG, 'hex'), { dkLen: 32 })).toString('hex') + '87';
const TAG = Buffer.from(blake3('__covenant_entrypoint_auth_sell(sig,byte[32],byte,int,int)').slice(0, 4));
...
(async () => {
  ...
  const V = 1000000000n, P = 100000000n;
  const inputs: UtxoInput[] = [{ txId: genesis, index: 0, sequence: 0, spk: SPK, amount: V }, w];
  const outputs: (TxOutput & { covenant?: any })[] = [
    { amount: V, scriptPublicKey: SPK, covenant: { authorizingInput: 0, covenantId: COV } },
    { amount: P, scriptPublicKey: `20${USER}ac` }, { amount: w.amount - P - 2000000n, scriptPublicKey: `20${USER}ac` }];
  const s0 = Buffer.from(secp.schnorr.signSync(kaspa_sighash_v1(inputs, outputs, 0, 0n), PRIV)).toString('hex') + '01';
  const s1 = '41' + Buffer.from(secp.schnorr.signSync(kaspa_sighash_v1(inputs, outputs, 1, 0n), PRIV)).toString('hex') + '01';
  const sb = new SB(); sb.data(Buffer.from(s0, 'hex')); sb.data(Buffer.from(USER, 'hex')); sb.data(Buffer.from([0])); sb.int(P); sb.int(1n); sb.data(TAG); sb.data(Buffer.from(PROG, 'hex'));
  const tx = { version: 1, inputs: [
    ...
    { previousOutpoint: { transactionId: w.txId, index: w.index }, signatureScript: s1, sequence: 0, sigOpCount: 0, computeBudget: 10 }],
  ...
  const body = { ...tx, gas: 0, inputs: tx.inputs.map((i: any, k: number) => ({ ...i, utxo: { amount: Number(k === 0 ? V : w.amount), scriptPublicKey: { version: 0, script: k === 0 ? SPK : `20${USER}ac` }, ...(k === 0 ? { covenantId: COV } : {}) } })),
  ...
  if (pf.verdict !== 'ready') { console.log(JSON.stringify(pf.findings)); process.exit(1); }
  ...
  for (const url of ['wss://electron-10.kaspa.stream/kaspa/testnet-10/wrpc/json', 'wss://vector-10.kaspa.green/kaspa/testnet-10/wrpc/json']) {
    const ok = await new Promise<boolean>(res => { const ws = new WS(url, { headers: { 'User-Agent': 'Mozilla/5.0', 'Origin': 'https://wallet.kaspanet.io' } });
      ...
      ws.on('message', (d: any) => { const s = d.toString(); console.log('📥', s.substring(0, 200)); clearTimeout(t);
        if (s.includes('transactionId')) { console.log(' REVEALED:', JSON.parse(s).params.transactionId); res(true); } else res(false); });
