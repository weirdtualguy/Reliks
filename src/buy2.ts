...
(async () => {
  const cont = compileState(0); // Next state: buyer owned, price 0
  ...
  const coinValue = BigInt(state.value);
  // THE HACK: Combine seller + royalty to avoid 4-output storage mass penalty
  const sellerPayout = PRICE + (PRICE / 50n); // 1.02 TKAS
  const change = w.amount - sellerPayout - 2000000n; // 2M fee

  const inputs: UtxoInput[] = [
    { txId: state.txid, index: 0, sequence: 0, spk: state.spk, amount: coinValue },
    w
  ];
  
  const outputs = [
    out(coinValue - 1000000n, cont.scriptPublicKey, { authorizingInput: 0, covenantId: state.covenantId }),
    out(sellerPayout, `20${USER}ac`),
    out(change, `20${USER}ac`)
  ];

  // Build buy sigscript: [buyerId, buyerScheme, paymentOutIdx, royaltyOutIdx, tag, program]
  const sb = new SB();
  sb.data(Buffer.from(USER, 'hex')); // buyerIdentifier (self-buy for demo)
  sb.data(Buffer.from([0]));         // buyerScheme
  sb.int(1n);                        // paymentOutIdx
  sb.int(1n);                        // royaltyOutIdx (same index to satisfy both checks)
  sb.data(TAG_BUY);
  sb.data(Buffer.from(state.programHex, 'hex'));
  ...
  const tx = { version: 1, inputs: [
      ...
      { previousOutpoint: { transactionId: w.txId, index: w.index }, signatureScript: fsig, sequence: 0, sigOpCount: 0, computeBudget: 10 }],
  ...
  fs.writeFileSync('plan.json', JSON.stringify({ transaction: tx,
    utxos: [{ amount: state.value, spk: state.spk, covenantId: state.covenantId }, { amount: w.amount.toString(), spk: w.spk }],
    next: { programHex: cont.bytecodeHex, spk: cont.scriptPublicKey, value: Number(coinValue - 1000000n), price: 0 } }));
  console.log('✅ plan.json (trustless buy @ 1.02 TKAS total) ready');
})();
