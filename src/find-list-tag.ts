...
(async () => {
  ...
  const candidates = [
    { name: '__covenant_entrypoint_auth_list', tag: tag('__covenant_entrypoint_auth_list', ['sig', 'int']) },
    { name: 'list', tag: tag('list', ['sig', 'int']) },
    { name: '__list', tag: tag('__list', ['sig', 'int']) },
  ];

  for (const c of candidates) {
    console.log(`\n🔍 Trying tag: ${c.name} (${c.tag.toString('hex')})`);
    const sb = new SB();
    ...
    sb.data(Buffer.from(sig, 'hex')); sb.int(PRICE); sb.data(c.tag); sb.data(Buffer.from(state.programHex, 'hex'));
    ...
    console.log(`   preflight: ${pf.verdict} | units: ${pf.executed?.[0]?.script_units_used || 0} | msg: ${pf.executed?.[0]?.verdict || pf.findings?.[0]?.message}`);
  ...
  console.log('\n❌ No tag worked. Try adding more candidates to the list.');
  process.exit(1);
})();
