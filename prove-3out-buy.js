...
function pushMinimalInt(val) {
  if (val === 0) return B.from([0x00]);
  if (val >= 1 && val <= 16) return B.from([0x50 + val]);
...
(async () => {
  const covid = process.argv[2];
  if (!covid) { 
    console.log('Usage: node prove-3out-buy.js <covenantId>'); 
    console.log('NOTE: Edition must already be listed by the artist (USER)!'); 
    process.exit(1); 
  }
  ...
  if (ed.ownerIdentifier !== USER) { console.error('❌ Edition not owned by USER (artist). 3-output proof requires owner == artist.'); process.exit(1); }
  if (ed.artist !== USER) { console.error('❌ Edition artist is not USER. 3-output proof requires owner == artist.'); process.exit(1); }
  
  const price = Number(ed.price);
  if (!(price > 0)) { 
    console.error('❌ Edition not listed. Run `node marketplace-v3.js list ' + covid + ' <price>` first and wait for confirmation.'); 
    process.exit(1); 
  }

  console.log('Current state:', ed);
  console.log('Proving 3-output buy (owner == artist, no royalty output)...');
  ...
  const out1 = BigInt(price);                         // Seller payment (who is also artist)
  // NO out2 for royalty!
  const totalCost = out1;
  ...
  const outputs = [
    ...
    { amount: out1, scriptPublicKey: '20' + ed.ownerIdentifier + 'ac' }, // Seller/Artist
    { amount: change, scriptPublicKey: wInput.spk }                      // Buyer change
  ];

  const inputs = [{ txId, index, sequence: 0, spk: oldSpk, amount }, wInput];

  // Sigscript for buy: buyer, buyerScheme, paymentOutIdx, royaltyOutIdx, tag, redeem
  // We pass 1 for royaltyOutIdx. Since owner == artist, the covenant's `if` block is skipped,
  // so it won't evaluate tx.outputs[1] for royalty requirements.
  const sig0script = hex(B.concat([
    pushExplicit(H(USER)),      // buyer
    pushExplicit(B.from([0])),  // buyerScheme
    pushMinimalInt(1),          // paymentOutIdx
    pushMinimalInt(1),          // royaltyOutIdx (dummy)
  ...
  const rpcTx = {
    ...
    outputs: outputs.map(o => ({
      value: Number(o.amount), scriptPublicKey: '0000' + o.scriptPublicKey, ...(o.covenant ? { covenant: o.covenant } : {})
    })),
  ...
  if (out) {
    ed.ownerIdentifier = USER; ed.identifierType = 0; ed.price = 0;
    fs.writeFileSync(ledgerPath, JSON.stringify(editions, null, 2));
    console.log('✅ 3-OUTPUT BUY PROOF SUCCESS! TxId:', out);
    console.log(`🔗 https://kascov.io/testnet-10/tx/${out}`);
    console.log('✅ Ledger updated (royalty output successfully omitted)');
  } else {
