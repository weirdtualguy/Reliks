...
v5 MULTI-CHUNK ERA — breaking the 2048-byte limit (2026-09-17)
...
- mint-edition/: early Rust mint experiment (pre-JS-toolchain era), 195K scratch crate;
  absorbed as plain files on 2026-09-17 after removing its zero-commit nested .git.
v5 GALLERY RETRIEVAL — trustless Genesis Link Pattern (2026-09-17)
web/v5-gallery.html renders v5 editions with zero trusted servers: registry pointer -> REST reveal tx ->
sigscript push parsing -> 8x2048B chunks -> blake2b(32B) == program_hash read from factory live state
on-chain (mint tx input0 redeem, state_span slice). Verified d425b2e1... match; hard-fail on mismatch.
Files: web/v5-gallery.html, web/v5-registry.json (gen-v5-registry.js), web/factory-abi-v5.json,
patch-gallery-trustless.js. blake2b dkLen:32 gotcha pinned (noble default digest is 64 bytes).

KCC20 ERA — PixelToken/PixelMinter composition proven (2026-09-17)
C=030582e0… (genesis f867f412); A=1a1ff247… (asset genesis+init ab8f2fee); mint a823db4d
(1000 PCRT, allowance 999000); transfer b346047c (supply conserved); over-mint negative rejected.
Files: PixelToken.sil, PixelMinter.sil, pixel-{token,minter}-abi.json, token-ledger.json,
pcrt-recipient.json, deploy-token.js, mint-token.js, transfer-token.js, overmint-negative.js (+patchers).
ICC model proven: covenant-ID ownership (type 0x02) + readInputStateWithTemplate /
validateOutputStateWithInputTemplate cross-template validation; issuance policy external to token.

DECL ERA — SeriesFactory-v6 macro-lowered mint proven (2026-09-17)
v6 factory genesis 01f90235 (C=de2943ed...); Edition #0 minted f0777598 (E=fa37c93f...).
Refactored v5 hand-written OpAuth* routing into DECL macros. `mint` is now an auth-bound singleton
transition policy. Compiler wrapper handles cardinality + validateOutputState; policy body handles
foreign-template Edition spawn. Public ABI (tags/params) byte-identical to v5; mint-art-v6.js needed
zero codec changes. Files: SeriesFactory-v6.sil, factory-abi-v6.json, factory-ledger-v6.json,
diff-factory-v5-v6.js.

OFFER ESCROW ERA CLOSED (2026-09-17)
accept b08f2ee2 (owner paid exact escrow value; royalty via Edition sell); positive permissionless
expire 3158320e (no sig in escrow input); negative early-expire c9ee9e9d rejected by consensus
sequence lock ("sequence locks conditions was not met") — two-layer age enforcement.
Usage: deploy-offer.js <editionCovId> <amount> <expireAge>; accept-offer.js <edTx:idx> [sell|transfer];
expire-offer.js (permissionless). offer-ledger.json/offer-keys.json are per-run caches; back up
offer-keys.json before redeploying whenever the old offerer key owns an edition.

GALLERY V7 CLOSED (2026-09-17): registry key-tolerant (covenantId||C) + skip-unrevealed; 1b uses
kascov live-utxo with REST mintTx-hint fallback; parsePushes(redeem)[0] version-agnostic extraction.
Renders v5 (0c3cab42, 1 reveal) and v7 (e9ae4dbf, 4 reveals) identically; v6 unrevealed => skipped.
Open testnet items: v6 reveal (optional), fork non-conservation negative cert, stranded offer
3f6dde00 recoverable after ~3000 DAA via expire-offer.js. Mainnet prep pending.
