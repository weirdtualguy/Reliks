...
(async () => {
  const F = parts(JSON.parse(fs.readFileSync('factory-abi-v4.json', 'utf8')));
  const E = parts(JSON.parse(fs.readFileSync('edition-abi-v3.json', 'utf8')));
  const LEDGER = JSON.parse(fs.readFileSync('factory-ledger-v4.json', 'utf8'));
  ...
  const fState = c => ({ art: ART, artist: USER, price: 100000000, royalty_bips: 500, cap: 64, counter: c });
  ...
  if (txId) {
    ...
    fs.writeFileSync('factory-ledger-v4.json', JSON.stringify(LEDGER, null, 2));
