...
const F = JSON.parse(fs.readFileSync('factory-abi-cap2.json', 'utf8'));
...
const TAG = H(c.entries.close.dispatch_tag);
console.log('close tag:', c.entries.close.dispatch_tag);
(async () => {
  const L = JSON.parse(fs.readFileSync('factory-ledger-cap2.json', 'utf8'));
  const ART = fs.readFileSync('art-program.hex', 'utf8').trim();
  const fState = k => ({ art: ART, artist: USER, price: 100000000, royalty_bips: 500, cap: 2, counter: k });
  const oldSpk = p2sh(B.concat([prefix, encState(fState(L.counter)), suffix]));
  let utxo;
  for (let i = 0; i < 24; i++) {
    const cov = await (await fetchRetry('https://kascov.io/data/testnet-10/c/' + L.covenantId + '.json')).json();
  ...
  // Terminal spend: factory value reclaimed to artist P2PK, wallet pays the fee
  const outputs = [
    { amount: fAmt, scriptPublicKey: '20' + USER + 'ac' },
    { amount: wIn.amount - 2000000n, scriptPublicKey: wIn.spk }
  ];
  const inputs = [{ txId, index: parseInt(idx), sequence: 0, spk: oldSpk, amount: fAmt }, wIn];
  const sig0 = B.concat([secp.schnorr.signSync(sighash(inputs, outputs, 0), PRIV), B.from([0x01])]);
  const sig0script = hex(B.concat([pushExplicit(sig0), pushExplicit(TAG),
    pushExplicit(B.concat([prefix, encState(fState(L.counter)), suffix]))]));
  ...
  if (out) console.log('🔓 FACTORY CLOSED — storage mass reclaimed:', out);
