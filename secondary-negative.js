...
(async () => {
  const LD = JSON.parse(fs.readFileSync('data/factory-ledger-v10-market.json', 'utf8'));
  const Ed = parts(JSON.parse(fs.readFileSync('data/edition-abi-v5.json', 'utf8')));
  ...
  if (cmd === 'list') {
    ...
    fs.writeFileSync('data/factory-ledger-v10-market.json', JSON.stringify(LD, null, 2));
    ...
    function build(feeTx) {
      ...
      const ss = B.concat([pushMin(H(V.USER)), pushMinInt(1), pushMinInt(2), pushMinInt(2), pushMin(TAG('buy')), pushMin(curRedeem)]);
    ...
    function build(feeTx) {
      ...
      const sigW1 = '41' + hex(secp.schnorr.signSync(sighash(inputs, outputs, 1), V.PRIV)) + '01';
      const ss = B.concat([pushMin(H(buyer)), pushMinInt(askPrice), pushMinInt(1), pushMinInt(2), pushMinInt(3), pushMin(B.concat([ownerSigRaw, B.from([0x01])])), pushMin(TAG('sell')), pushMin(curRedeem)]);
      return rpc(inputs, outputs, hex(ss), sigW1);
    }
    const { txId } = await feeLoop(build);
    await waitForConfirmation(txId);
