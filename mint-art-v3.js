...
(async () => {
  const F = parts(JSON.parse(fs.readFileSync('factory-abi-v3.json', 'utf8')));
  const E = parts(JSON.parse(fs.readFileSync('edition-abi-v3.json', 'utf8')));
  const LEDGER = JSON.parse(fs.readFileSync('factory-ledger-v3.json', 'utf8'));
  ...
  if (txId) {
    ...
    fs.writeFileSync('factory-ledger-v3.json', JSON.stringify(LEDGER, null, 2));
